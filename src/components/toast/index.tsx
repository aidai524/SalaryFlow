...
export default function Toast(props: ToastProps) {
  ...
  return (
    <div
      className={cn(
        "flex w-72 max-md:w-[calc(100vw-32px)] items-start gap-2 rounded-xl bg-white px-2.5 py-2 shadow-[0_0_6px_0_rgba(0,0,0,0.10)] md:w-72",
        className,
      )}
    >
      <ToastIcon type={type} />
      <div className="flex grow items-start justify-between font-space-grotesk text-[#444C59]">
        <div className="flex flex-col gap-0.5">
          <div className="items-center text-sm font-normal leading-normal">{title}</div>
          {text ? <div className="text-xs font-light leading-normal">{text}</div> : null}
        </div>
        <button
          type="button"
...
function ToastIcon(props: { type: ToastType }) {
  ...
  if (type === ToastType.Pending) {
    return <IconProcessing className="size-4 shrink-0 translate-y-[3px] animate-spin text-[#6284F5]" />;
  }
  ...
  return (
    ...
    >
      ...
      <line
        ...
        x2="16.4706"
        y2="-0.5"
        transform="matrix(-0.707107 -0.707107 -0.707107 0.707107 12.499 12.5)"
        stroke="#444C59"
        strokeLinecap="round"
